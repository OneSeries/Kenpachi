    ...
    Reduce { state, action in
      ...
      case .refresh:
        ...
        return .run { send in
          ...
          await send(.watchlistLoaded([]))
          await send(.watchHistoryLoaded([]))
          await send(.statisticsLoaded(watchTime: 0, contentCount: 0, genres: []))
        }
      ...
      case .settings(.presented(.delegate(.settingsUpdated))):
        /// Settings were updated, notify parent
