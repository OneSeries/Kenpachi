...
struct AnimeKai: ScraperProtocol {
  ...
  func fetchContentDetails(id: String, type: ContentType?) async throws -> Content {
    ...
    let malId = try? watchSection?.attr("data-mal-id")
  ...
  /// Note: This is a placeholder - implement actual decoding logic based on your backend
  private func fetchDecodedToken(animeId: String) async throws -> String? {
    // In the Kotlin code, this calls BuildConfig.KAISVA/?f=e&d={animeId}
    // You'll need to implement this endpoint or remove if not needed
    return nil
  }
    ...
    async throws -> [ExtractedLink]
  {
    ...
    let linksURL = "\(baseURL)/ajax/links/list?token=\(episodeToken)"
    ...
    for server in servers {
      ...
      let viewURL = "\(baseURL)/ajax/links/view?id=\(lid)"
      ...
      guard let viewResponse = try? JSONDecoder().decode(LinksResponse.self, from: viewData),
        let iframeURL = extractIframeURL(from: viewResponse.result)
      else { continue }
